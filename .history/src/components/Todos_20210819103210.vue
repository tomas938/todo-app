		...
		<ul class="todos">
			<li v-for="(todo, index) in filteredTodos" :key="index" class="todo-item">
				<div
					...
					@click="
						(e) =>
							e.target.classList.toggle('complete') && addCompleteTodo(todo)
					"
				></div>
				<span>{{ todo }}</span>
				<svg
			...
			<div class="filters">
				...
				<div class="filter">
					<span :class="{ active: all }" @click="toggleAll()">All</span>
					<span :class="{ active: active }" @click="toggleActive()"
						>Active</span
					>
...
export default {
	...
	methods: {
		...
		addCompleteTodo(todo, index) {
			if (this.completedTodos.some((val) => val === todo)) {
		...
		toggleAll() {
			...
			this.completed = false;
			this.all = true;
			this.allTodos = this.todos.concat(this.completedTodos);
		},
	},
	computed: {
		filterAll() {
		retunn	if (this.filter === "all") {
				return this.todos;
			}
		},
	},
